...
# ARXC_INCLUDE_PREFIXES_FILE is optional, pass "" if the package includes need no prefixes.
function (acf_custom_build_ext PROJECT_BINARY_DIR ARXC_FILES ARXC_CONFIG ACF_CONVERT_REGISTRY ACF_CONVERT_CONFIG ACF_CONVERT_FILES ACF_ENV_VARS ACF_ARX_DEPS A_PROJECT_NAME ARXC_INCLUDE_PREFIXES_FILE)
	...
	# Arxc reads the include prefixes file, so a change of it has to re-run the code generation.
	set(ARXC_INCLUDE_PREFIXES_ARG)
	if(ARXC_INCLUDE_PREFIXES_FILE)
		set(ARXC_INCLUDE_PREFIXES_ARG -include_prefixes ${ARXC_INCLUDE_PREFIXES_FILE})
	endif()
	string(REPLACE ";" " " ARXC_INCLUDE_PREFIXES_TEXT "${ARXC_INCLUDE_PREFIXES_ARG}")

	# NOTE: OUTPUT order must be same as in the DEPFILE
	add_custom_command(
		OUTPUT ${HEADER_FILE_AUX} ${SOURCES_FILE_AUX}
		COMMAND ${ARXCBIN}
		ARGS ${ARXC_FILES} -o ${ARXC_OUTFILE} -d ${ARXC_OUTFILE}.d -config ${ARXC_CONFIG} -conf_name ${CMAKE_BUILD_TYPE}_${TARGETNAME} -env_vars ${ACF_ENV_VARS_SINGLE_STRING} ${ARXC_INCLUDE_PREFIXES_ARG}
		DEPENDS ${ARXCBIN} ${ARXC_FILES} ${ARXC_INCLUDE_PREFIXES_FILE}
		DEPFILE ${ARXC_OUTFILE}.d
		VERBATIM
		COMMENT "ArxcBinStarts: ${ARXCBIN} ${ARXC_FILES} -o ${ARXC_OUTFILE} -d ${ARXC_OUTFILE}.d -config ${ARXC_CONFIG} -conf_name ${CMAKE_BUILD_TYPE}_${TARGETNAME} -env_vars ${ACF_ENV_VARS_SINGLE_STRING} ${ARXC_INCLUDE_PREFIXES_TEXT}"
	)
