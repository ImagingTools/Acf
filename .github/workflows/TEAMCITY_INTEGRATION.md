...
| `TEAMCITY_BUILD_TYPE_WINDOWS_PR` | TeamCity build configuration ID for Windows pull requests | `Acf_Build_Windows` |
| `TEAMCITY_BUILD_TYPE_WINDOWS_MAIN` | TeamCity build configuration ID for Windows `main`/`master` pushes | `Acf_Build_Windows_Main` |
| `TEAMCITY_BUILD_TYPE_LINUX_PR` | TeamCity build configuration ID for Linux pull requests | `Acf_Build_Linux` |
| `TEAMCITY_BUILD_TYPE_LINUX_MAIN` | TeamCity build configuration ID for Linux `main`/`master` pushes | `Acf_Build_Linux_Main` |
...
4. Examples:
   - Windows pull request: `Acf_CMakeBuild_Windows`
   - Windows `main`/`master`: `Acf_CMakeBuild_Windows_Main`
   - Linux pull request: `Acf_CMakeBuild_Linux`
   - Linux `main`/`master`: `Acf_CMakeBuild_Linux_Main`
...
- The TeamCity token is stored as a GitHub repository **secret** (`secrets.TEAMCITY_TOKEN`), not a variable, so it is never exposed in logs or the Actions UI
