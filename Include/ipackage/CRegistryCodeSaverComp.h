...
public:
	...
	I_BEGIN_COMPONENT(CRegistryCodeSaverComp);
		...
		I_ASSIGN(m_translationLevelAttrPtr, "TranslationLevel", "Level of configuration used for translation all packages, if disabled all levels of used packages will be stored", false, 0);
		...
		I_ASSIGN(m_depfilePathAttrPtr, "DepfilePath", "Optional path where the dependency file will be stored", false, "");
		I_ASSIGN_MULTI_0(m_includePrefixesAttrPtr, "PackageIncludePrefixes", "List of package include prefixes in form 'PackageId=Prefix', where the prefix is the directory of the package header relative to an include path. Includes of listed packages will be generated as 'Prefix/PackageId.h' instead of 'PackageId/PackageId.h'", false);
	I_END_COMPONENT;
...
protected:
	...
	typedef QMap<QByteArray, QString> IncludePrefixes;
	...
	*/
	...
	/**
		Get include prefixes per package ID as configured in \c PackageIncludePrefixes.
	*/
	IncludePrefixes GetIncludePrefixes() const;

	/**
		Get path of package header file used in generated include directives.
		It returns 'Prefix/PackageId.h' for packages having an include prefix, otherwise 'PackageId/PackageId.h'.
	*/
	QString GetPackageIncludePath(const QByteArray& packageId, const IncludePrefixes& prefixes) const;
...
private:
	...
	I_ATTR(int, m_translationLevelAttrPtr);
	I_ATTR(bool, m_useBinaryCodeAttrPtr);
	I_ATTR(QString, m_depfilePathAttrPtr);
	I_MULTIATTR(QString, m_includePrefixesAttrPtr);
