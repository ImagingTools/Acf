...
*/
template <class InterfaceType, class Key = std::string>
class TComposedFactory: virtual public TIFactory<InterfaceType, Key>
{
public:
	typedef TIFactory<InterfaceType, Key> FactoryInterface;
	...
	*/
	...
	virtual InterfaceType* CreateInstance(const Key* keyPtr = NULL) const;
...
template <class InterfaceType, class Key>
bool TComposedFactory<InterfaceType, Key>::RegisterFactory(FactoryInterface* factoryPtr, bool releaseFlag)
{
...
template <class InterfaceType, class Key>
IFactoryInfo::KeyList TComposedFactory<InterfaceType, Key>::GetFactoryKeys() const
{
...
template <class InterfaceType, class Key>
InterfaceType* TComposedFactory<InterfaceType, Key>::CreateInstance(const Key* keyPtr) const
{
	for (		typename FactoryList::const_iterator iter = m_factoryList.begin();
				iter != m_factoryList.end();
				++iter){
		...
		InterfaceType* createdPtr = factoryPtr->CreateInstance(keyPtr);
