...
void CListObserverDispatcher::DispatchListChange(const istd::IChangeable::ChangeSet& changeSet)
{
	if (changeSet.Contains(istd::IChangeable::CF_ALL_DATA) || changeSet.Contains(CObservableListBase::CF_RESET) 
		...
		OnListReset();
		return; // do not process other changes if cumulative changeSet contains reset
	}
...
{
	CListObserverDispatcher::DispatchListChange(changeSet);
}
