...
void CSimpleXmlFileArchiveTest::DoPersistenceComponentTest()
{
	...
	{
		int state = component.WriteToFile(filePathParam, testFilePath);
		QVERIFY(state == ifile::IFilePersistence::Successful);
	}

	imod::TModelWrap<ifile::CFileNameParam> filePathParam2;
	int state = component.ReadFromFile(filePathParam2, testFilePath);
	QVERIFY(state == ifile::IFilePersistence::Successful);
